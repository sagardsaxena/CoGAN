...
class CoGanTrainer(object):
    def __init__(self, batch_size=64, latent_dims=100):
        super(CoGanTrainer, self).__init__()
        self.dis = CoDis28x28()
        self.gen = CoGen28x28(latent_dims)
        self.dis_opt = torch.optim.Adam(self.dis.parameters(), lr=0.0002, betas=(0.5, 0.999), weight_decay=0.0001)
        self.gen_opt = torch.optim.Adam(self.gen.parameters(), lr=0.0002, betas=(0.5, 0.999), weight_decay=0.0001)
    ...
    def load_weights(self, gen_pkl, dis_pkl):
        self.gen.load_state_dict(torch.load(gen_pkl))
        self.dis.load_state_dict(torch.load(dis_pkl))

    def dis_update(self, images_a, labels_a, images_b, noise, mse_wei, cls_wei):
        images_a, images_b = images_a.float(), images_b.float()
        self.dis.zero_grad()
        ...
        dummy_tensor = Variable(
            torch.zeros(fake_feat_a.size(0), fake_feat_a.size(1))).cuda()#, fake_feat_a.size(2), fake_feat_a.size(3))).cuda()
